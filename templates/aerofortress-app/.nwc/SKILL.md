...
1. Run `dotnet tool run af nwc wake` at task start, after context reset or compaction, and when
   the host supplies a new event. Due deferments are obligations.
2. Use `dotnet tool run af nwc resolve --id <id> --evidence "<proof>"` only after completing the
   deferred action.
3. The host runs `dotnet tool run af nwc collect --task "<goal>" --plan <file> --final-message <file>`
