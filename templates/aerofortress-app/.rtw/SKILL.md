...
1. At task start, run `dotnet tool run af rtw guide --task "<goal>" --path <expected-path>` before editing. Read every returned way and inspect each referenced file.
2. Rerun `dotnet tool run af rtw guide` when scope changes, context is reset or compacted, an unfamiliar area is entered, or review begins.
...
4. Run `dotnet tool run af rtw add` only after a reusable pattern exists in tracked repository code. Provide a precise intent, actionable guidance, reusable scopes and tags, and at least one reference.
5. Before committing an uncommitted final diff, run `dotnet tool run af rtw check --task "<completed task>"`.
...
Do not report work ready until the applicable `dotnet tool run af rtw check` exits with code 0.
