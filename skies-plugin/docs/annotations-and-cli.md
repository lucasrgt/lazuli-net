...
- `[JsonConverter(typeof(ScalarJsonConverter<TVo, TPrim>))]` — scalar VO crosses the wire as its primitive; OpenAPI schema mirrored by `AddSkiesOpenApi`.
...
- `skies new [name]` — full project (Skies.toml, src/<App>.Api, Program.cs index, sample module, Modules.cs).
- `skies foundations init [--agent-file <path>]...` — initialize the versioned NYA, WTW, RTW, and NWC
  protocols through the framework-pinned binaries.
- `skies g module <Name>` — module + `<Name>.ctx.md` + entities placeholder.
- `skies g slice <Module> <Name> --verify <id,id>` — slice + co-located test + complete write
  journeys + manifest declaration + exact AVP proof scaffold. The criterion list is mandatory.
- `skies g auth` — auth module (Login/Refresh/Register + journeys), Identity entity, session seam.
- `skies g hub <Module> <Name>` — SignalR hub at `Modules/<Module>/Realtime/<Name>Hub.cs` (hub is wire: calls the slice, fans out).
- `skies doctor` — diagnostic structural check: Roslyn SKY* + Skies.toml validation + frontend SKYFE*.
- `skies gate` / `--affected --base <rev>` — the default done verdict: doctor + universal inventory + the complete
...
- `skies gate --staged --fast` — bounded index-rooted pre-commit feedback; mapped proofs run, while exhaustive
  fallbacks and browser/device execution wait for authoritative CI.
- `skies gate --affected --base <rev> --fast` — bounded local pre-push feedback over the commits being sent.
- `skies gate --full` — exhaustive release audit over every declared proof.
- `skies nya <args...>` — pinned Not You Again CLI for scar recall, collection, specification review,
  recurrence checks, and replay.
- `skies wtw <args...>` — pinned Why This Way CLI for governing decisions, invariants, collection,
  explanation, graph health, and final guards.
- `skies rtw <args...>` — pinned Right This Way CLI for proven-pattern guidance, recording, and alignment checks.
- `skies nwc <args...>` — pinned Now We Can CLI for collecting, waking, resolving, and checking conditional deferments.
- `skies test [--unit|--integration|--e2e]` — the .NET test leg, optionally filtered by category.
