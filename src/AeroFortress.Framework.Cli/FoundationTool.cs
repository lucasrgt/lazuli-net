...
internal sealed class FoundationTool
{
    ...
    internal int Run(string[] arguments)
    {
        ...
        {
            Console.Error.WriteLine($"af {Id}: {exception.Message}");
    ...
    {
        var supplied = Environment.GetEnvironmentVariable($"AEROFORTRESS_{Id.ToUpperInvariant()}_BINARY");
        if (!string.IsNullOrWhiteSpace(supplied))
        {
            var explicitPath = Path.GetFullPath(supplied);
            if (!File.Exists(explicitPath))
                throw new InvalidOperationException(
                    $"AEROFORTRESS_{Id.ToUpperInvariant()}_BINARY does not exist: {explicitPath}");
            return explicitPath;
        }
        ...
        var temporary = Path.Combine(Path.GetTempPath(), $"af-{Id}-" + Guid.NewGuid().ToString("N"));
        try
        {
            ...
            Console.WriteLine($"af {Id}: installed verified {DisplayName} {Version} for {target}.");
            return binary;
        }
        finally
        {
    ...
    {
        var configured = Environment.GetEnvironmentVariable($"AEROFORTRESS_{Id.ToUpperInvariant()}_HOME");
        ...
        return Path.Combine(cache, "aerofortress", "tools", Id);
    }
