...
internal static class DoctorCommand
{
    ...
    /// <c>skies gate</c> always enables this final-state policy.
    /// </param>
    public static int Run(string[] rest, bool strictWarnings = false)
    {
        ...
        var manifest = SkiesManifest.Validate(root);
        Console.WriteLine("skies doctor — manifest (Skies.toml)...");
        ...
        Console.WriteLine("skies doctor — framework sync...");
        ...
        var legs = new List<Func<int>>
        {
            () =>
            {
                Console.WriteLine("skies doctor — backend conventions (build)...");
        ...
        foreach (var client in FrontendTargets(root))
        {
            ...
            legs.Add(() =>
            {
                Console.WriteLine($"skies doctor — frontend lint ({Path.GetFileName(captured.Path)})...");
                return FrontendScriptContract.Run(captured.Path, "lint");
            });
            legs.Add(() =>
            {
                Console.WriteLine($"skies doctor — frontend typecheck ({Path.GetFileName(captured.Path)})...");
