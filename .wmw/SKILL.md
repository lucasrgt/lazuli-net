...
1. Run `dotnet tool run af wmw wake` at task start, after context reset or compaction, and when
   the host supplies a new event. Due deferments are obligations.
2. Use `dotnet tool run af wmw resolve --id <id> --evidence "<proof>"` only after completing the
   deferred action.
3. The host runs `dotnet tool run af wmw collect --task "<goal>" --plan <file> --final-message <file>`
