...
public class NyaCommandTests
{
    ...
    public void Init_adaptation_keeps_agent_commands_on_the_framework_pinned_wrapper()
    {
        ...
        Assert.Contains("`dotnet tool run skies nya recall`", File.ReadAllText(Path.Combine(root, ".nya", "SKILL.md")));
        Assert.Contains("`dotnet tool run skies nya spec`", File.ReadAllText(Path.Combine(root, ".nya", "SKILL.md")));
        Assert.Contains("`dotnet tool run skies nya replay`", File.ReadAllText(Path.Combine(root, ".nya", "SKILL.md")));
        Assert.Contains("`dotnet tool run skies nya check`", File.ReadAllText(Path.Combine(root, "AGENTS.md")));
    }
    ...
    public void Project_validation_requires_the_versioned_store_and_agent_protocol()
    {
        ...
        File.WriteAllText(
            Path.Combine(root, ".nya", "SKILL.md"),
            """
            Run `dotnet tool run skies nya recall`.
            Run `dotnet tool run skies nya spec`.
            Run `dotnet tool run skies nya check`.
            Run `dotnet tool run skies nya replay`.
            """);
        File.WriteAllText(Path.Combine(root, "AGENTS.md"),
    ...
    public void Project_validation_rejects_an_outdated_skill_protocol()
    {
        ...
        Directory.CreateDirectory(Path.Combine(root, ".nya", "scars"));
        File.WriteAllText(Path.Combine(root, ".nya", "config.toml"), "schema = 1");
        File.WriteAllText(Path.Combine(root, ".nya", "SKILL.md"), "Run `dotnet tool run skies nya recall`.");
        File.WriteAllText(
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "skies-nya-" + Guid.NewGuid().ToString("N"));
